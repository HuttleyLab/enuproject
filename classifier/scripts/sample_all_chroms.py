...
def make_param_groups(flank_sizes, chroms):
    ...
    for flank_size in flank_sizes:
        ...
        feature_by_chrom = product(feature_dimensions, chroms)
        groups.extend([(flank_size, fd, chrom) for fd, chrom in feature_by_chrom])
    return groups

def make_command(exec_cmnd, args_template, basedir, flank_size, feature_dim, chrom):
    window_size = flank_size * 2 + 1
    outdir = os.path.join(basedir, "chrom%s/%d-mer/data_ratio_1" % (str(chrom), window_size))
    # these filenames actually have a .txt
    enu_path = "../data/ENU_variants/SNVs20151101_chrom%s" % str(chrom)
    germ_path = "../data/germline_variants/mouse_germline_All_88_chrom%s" % str(chrom)
    
    vals = dict(enu_data=enu_path, germ_data=germ_path,
                num_reps=1, flank_size=flank_size, feature_dim=feature_dim,
                size=0, outdir=outdir)
...
def main(resultdir, numprocs, test):
    ...
    commands = [make_command(sample, args_template, resultdir, fsz, fd, chrom)
                for fsz, fd, chrom in
                make_param_groups(flank_sizes=_flank_sizes, chroms=_chroms)]
