...
def make_param_groups(flank_sizes, chroms):
    groups = []
    for flank_size in flank_sizes:
        ...
        features = product(feature_dims, chroms)
        groups.extend([(chrom, flank_size, fd) for fd, chrom in features])
    return groups

def make_command(exec_cmnd, args_template, chrom, flank_size, feature_dim):
    ...
    clf_path = "../results/chrom1/%d-mer/data_ratio_1/direction_All/%d_way_samples/train_size_10255/sample_1/OCS/one_class_svm.pkl" % (window_size, feature_dim)
    outdir = "../results/chrom%s/%d-mer/data_ratio_1/direction_All/%d_way_samples/train_size_0/sample_1/OCS" % (chrom, window_size, feature_dim)
...
def main(numprocs, test):
    ...
    group = make_param_groups(flank_sizes=_flank_sizes, chroms=_chroms)
    
    commands = [make_command(sample, args_template, chrom, fsz, fd) for chrom, fsz, fd in make_param_groups(flank_sizes=_flank_sizes, chroms=_chroms)]
