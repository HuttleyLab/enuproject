...
def make_param_groups(flank_sizes, feature_dims, train_sizes, sample_idxs, clf_names):
    groups = []
...
def main(numprocs, test, gc_feature):
    ...
    if gc_feature:
        args = ["logreg_train_and_validate",
    ...
    _flank_sizes = [4, 8, 10, 20, 30, 40, 50]
    _train_sizes = [1009, 2050, 4101, 10255, 16408]
    _sample_idxs = [1, 2, 3, 4, 5]
    _feature_dims = [1]#2:memory error
    if gc_feature:
        _clf_names = ['LR_GC']
    if not gc_feature:
        _clf_names = ['LR']
    
    commands = [make_command(sample, args_template, fsz, fd, train_sz, idx, nm) for fsz, fd, train_sz, idx, nm in make_param_groups(flank_sizes=_flank_sizes, feature_dims=_feature_dims, train_sizes=_train_sizes, sample_idxs=_sample_idxs, clf_names=_clf_names)]
