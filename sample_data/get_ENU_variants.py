...
def get_snp_data(var_id, chromosome, coordinate, ref_base, var_base, effect, genome, flank_size):
    ...
    return (var_id, chromosome, strand, effect, allele_freqs, alleles,
                str(ref_base), str(var_base), str(f5), str(f3), str(gc_content), pep_alleles, gene_loc, gene_id, response)

@click.command()
@click.option('-i','--input_dir', help='Input directory containing mutation data.')
